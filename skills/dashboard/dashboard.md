...
Shows a unified coverage dashboard across all test scenarios — read-only view of what's covered, what has specs, and what needs work.
...
**Read `../_shared/directory-structure.md`** (relative to this skill's directory) for the filesystem layout.
...
|---------|-----------|--------|-------------|
| **Contracts - Create/Edit** — Form with contract number, entity, type, dates... | 10 | Spec ready | `/e2e:run contracts/create-edit` |
| **Contracts - List** — Filters, sorting, pagination, export | 12 | Spec ready | `/e2e:run contracts/list` |
| **Deals - Creation** — Multi-step wizard: details, quantities, review | 2 | Scenario only | `/e2e:run deals/creation` |
...
- **Next Action**: The command to run next, based on status:
  - **Spec ready** → `/e2e:run <area>/<name>` (run the spec)
  - **Scenario only** → `/e2e:run <area>/<name>` (generate + run the spec)
...
### Commands
  ...
  /e2e:run <area>/<scenario>                Run spec (generate if needed)
