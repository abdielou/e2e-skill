...
Scans the codebase for user-facing features and compares against existing scenarios to find coverage gaps. Produces `e2e-scenarios/COVERAGE.md`.
...
**Step 0: Run Phase -2 (Model Profile Check).** Read `../_shared/model-profiles.md` (relative to this skill's directory) and execute the "Phase -2: Model Profile Check" section. The current mode is `explore`.

**Then read** `../_shared/directory-structure.md` and `../_shared/rules-general.md` for context.
...
- If **Full rebuild**: Continue to Step 2 (full scan, overwrites existing catalog)
- If **Update a feature area**: Ask which feature area to update (show the areas from the existing catalog as options, plus "Other" for a new area). Then run Step 3 scoped to only that area, read the existing `COVERAGE.md`, and merge the updated section into it. Skip to Step 5 to write the merged catalog.
- If **Cancel**: Stop here.

**If the catalog does NOT exist**, proceed directly to Step 2.

---

### Step 2: Catalog Existing Coverage
...
When updating a single feature area (from Step 1), scope the scan to only that area's routes, components, and handlers instead of the full codebase.

**Model selection:** Set each subagent's `model` parameter based on the active profile (see Phase -2 model lookup table in `../_shared/model-profiles.md`).
...
### Step 5: Write Catalog & Display Report
...
> Last updated: [date]
>
> This file catalogs all known features. Status is inferred dynamically — run `/e2e:dashboard` to see the dashboard.
...
To see live status:  /e2e:dashboard
